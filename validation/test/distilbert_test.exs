defmodule EMLX.Validation.DistilBERTTest do
  @moduledoc """
  End-to-end validation tests for DistilBERT on EMLX.Backend.

  Mirrors Bumblebee's DistilBERT test suite — same tiny-random HuggingFace
  checkpoints, same inputs, same expected output slices. Reference values are
  from the HuggingFace Transformers (PyTorch) implementation; any divergence
  here is an EMLX bug on DistilBERT's critical path (embeddings, 6-block
  self-attention, LayerNorm, GELU FFN, task heads).
  ...
  test ":for_question_answering" do
    ...
    assert_all_close(outputs.start_logits[[.., 1..3]], Nx.tensor([[0.1790, -0.0074, 0.0412]]))
    assert_all_close(outputs.end_logits[[.., 1..3]], Nx.tensor([[-0.1520, -0.0973, 0.0166]]))
  end
