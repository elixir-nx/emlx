defmodule EMLXAxon.Llama.Generate do
  ...
  Generation supports immediate, deferred, and chunked host synchronization.
  Dense greedy states use the native Llama plugin while other sampler paths
  use tensor based forward calls.

  ## Usage

      {:ok, state} =
        EMLXAxon.Llama.DenseLoader.from_safetensors_dir(model_path, type: :f16)
  ...
  alias EMLXAxon.Llama.{Model, NativeChunkRunner, Sampler}
  ...
  - `:return_kv_cache` — when `true`, include the final KV cache in the returned
                         ...
                         the next request. When EOS is configured, cache-returning
                         `:end` synchronization falls back to `:per_token`, while
                         chunk synchronization uses one-token chunks so the cache
                         does not advance past the returned tokens.
  ...
  def generate_with_native_chunk_runner(
      ...
      when is_function(native_chunk_runner, 5) and is_function(timing_now, 0) do
    ...
    configured_max_len =
      opts
      |> Keyword.get(:max_len, @default_max_len)
      |> validate_max_len!()

    sampler =
      opts
      |> Keyword.get(:sampler, :greedy)
      |> validate_sampler!()
    ...
    host_sync =
      opts
      |> Keyword.get(:host_sync, :per_token)
      |> host_sync_mode(sampler, token_callback)
      |> cache_aligned_host_sync(return_kv_cache?, eos_id)
    ...
    decode_ctx =
      {eos_id, sampler, temp, top_p, state, profile_timing?, token_callback, chunk_callback,
       native_chunk_runner, timing_now}
  ...
  defp validate_top_p!(top_p) do
    raise ArgumentError,
          "expected :top_p to be a number greater than 0.0 and less than or equal to 1.0, " <>
            "got: #{inspect(top_p)}"
  end
  ...
  defp cache_aligned_host_sync(host_sync, false, _eos_id), do: host_sync
  defp cache_aligned_host_sync(host_sync, true, nil), do: host_sync
  defp cache_aligned_host_sync(:end, true, _eos_id), do: :per_token
  defp cache_aligned_host_sync({:chunk, _chunk_size}, true, _eos_id), do: {:chunk, 1}
  defp cache_aligned_host_sync(host_sync, true, _eos_id), do: host_sync
  ...
  defp decode_step_timed(n, acc_tokens, acc_times, kv, cur, rng_key, ctx, last_id) do
    {_eos_id, sampler, temp, top_p, state, profile_timing?, token_callback, _chunk_callback,
     _native_chunk_runner, timing_now} = ctx

    ts = if profile_timing?, do: timing_now.()
  ...
  defp decode_tensors(0, acc_tokens, acc_times, kv, cur, rng_key, ctx) do
    {eos_id, _sampler, _temp, _top_p, _state, _profile_timing?, _token_callback, _chunk_callback,
     _native_chunk_runner, _timing_now} = ctx
  ...
  defp decode_tensors(n, acc_tokens, acc_times, kv, cur, rng_key, ctx)
    ...
    {_eos_id, sampler, _temp, _top_p, state, profile_timing?, _token_callback, _chunk_callback,
     native_chunk_runner, _timing_now} = ctx
  ...
  defp decode_tensor_chunks_start(
         ...
         {eos_id, _sampler, _temp, _top_p, _state, _profile_timing?, _token_callback,
          chunk_callback, _native_chunk_runner, _timing_now} = ctx,
         chunk_size,
         true
       )
  ...
  defp decode_tensor_chunks(
    ...
    {eos_id, sampler, _temp, _top_p, state, profile_timing?, _token_callback, chunk_callback,
     native_chunk_runner, timing_now} = ctx
